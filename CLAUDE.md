...
- 종목 수가 적은(15 미만) 회차는 비정기이므로 달력에 넣지 않는다
...
### ⚠️ KIND 검색의 함정 2가지 — 둘 다 "조용히" 실패한다

실측으로 확인한 제약. 오류가 나지 않고 결과만 잘려서 나오므로 특히 위험하다.

| 제약 | 증상 | 대응 |
|---|---|---|
| **결과 100건 상한** | 초과분이 잘림. 최신순이라 **오래된 것부터 사라진다** | `kind_fetch.py`가 월 단위로 쪼개 검색 (한 달은 20건 안팎) |
| **검색 기간 3년 초과 시 0건** | 5년 범위로 검색하면 **오류 없이 0건** 반환 | 위와 동일 (월 단위 분할이 함께 막아줌) |
...
- **ETF 분배금 공시의 최초는 2009-09-07** (KIWOOM·ACE·TIGER·RISE 국고채3년 4건). 그 이전은 존재하지 않는다.
  - 2009 상반기에 `이익금분배`로 검색되는 건들은 ETF가 아니라 부동산·특별자산 펀드의 수익증권 분배다
  - ETF는 2002년부터 있었지만 이 공시 유형 자체가 2009년 9월에 생긴 것으로 보인다
- 연도별 공시량: 2009년 17건 → 2010년 65건 → 2012~2014년 각 100건+ → 이후 계속 증가
- 보유 현황: **2025-01부터** (2026-07-26에 2025-01~03 backfill 완료)
- 추가 backfill 시 **최근 연도부터 역순**(2024 → 2023 → …)이 효율적. 월배당 ETF가 늘어난 2020년 이후 구간이 상장이후 총수익률 개선 효과가 크다
