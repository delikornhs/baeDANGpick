...
## ⚠️ `--fetch-meta`가 메타 캐시를 통째로 지워 상장일 1,019개가 전멸했다

`update-prices.yml`은 **월요일마다** `etf_data_processor.py --fetch-meta`를 돌린다. 이 경로는 재조회를 강제하려고 `ETF_META_FILE.unlink()`로 **캐시 파일을 삭제**하고 시작했다.

문제는 캐시에 성격이 다른 두 종류가 섞여 있다는 점이다.

| 필드 | 출처 | 재취득 |
|---|---|---|
| `total_fee` `index_name` `summary` `holdings` | `m.stock.naver.com/api/etf/{code}/basic` (JSON API) | 매번 새로 받음 |
| `listed_date` `manager` | `finance.naver.com/item/main.nhn` **HTML 스크래핑** | 값이 있으면 건너뜀 |

상장일은 **캐시에 값이 없는 종목만** 스크래핑한다(`missing_date`). 캐시를 지우면 1,019개 전부가 "신규"가 되어 한꺼번에 스크래핑을 시도하고, 그 페이지가 바뀌거나 막히면 **전 종목이 빈 문자열로 덮인다.** 되돌릴 원본이 이미 삭제된 뒤다.

> 2026-09-14 사고: 상세페이지에 상장일이 안 나온다는 제보로 발견. 그날 실행(run #111)
> 로그에 `상장일 조회: 1019개 신규 종목` 다음 줄이 바로 `💾 메타 캐시 저장`이었다 —
> 1,019개를 돌고 **한 건도 못 읽었는데 오류 없이 성공으로 끝났다.**
> `latest.json`은 922 → 0, `etf_data.js`의 `listedDate`도 전부 `''`.
> JSON API 쪽(`total_fee` 등 922개)은 멀쩡해서 워크플로는 '성공'으로 표시됐다.
> `PRESERVE_FIELDS`는 도움이 안 된다 — `--prices-only` 경로가
> `item["listed_date"] = m.get("listed_date", "")`로 캐시 값을 무조건 덮어쓰기 때문.

**수정 (`etf_data_processor.py`에 적용됨):**
- `--fetch-meta`에서 **파일 삭제를 없앴다.** API로 다시 받는 4개 필드만 `pop`하고
  `listed_date`·`manager`는 남긴다
- 스크래핑 실패 시 **저장된 종가 이력에서 상장일을 복구**한다
  (`listed_date_from_history()`). `fetch_daily_price_history()`가 상장일을 모르면
  `startTime=19900101`로 전 구간을 받아오므로 **이력의 첫 거래일 = 상장일**이다.
  2026-09-07 캐시의 922종목과 대조해 **922/922 완전 일치**를 확인했다
- 조용히 실패하지 않도록 `상장일 확보: N/M개 (네이버 x / 종가이력 y)`를 찍고,
  네이버에서 **한 건도 못 읽으면 경고**를 출력한다
- `--prices-only`의 재조회 대상을 **"캐시에 키가 있는가"가 아니라 "값이 있는가"**로 바꿨다.
  종전 `new_codes = [c for c in codes if c not in meta_cache]`는 상장일이 빈 문자열로
  남은 종목을 영영 건너뛰었다 — 1,019개 전부 캐시에 *있긴* 했기 때문이다.
  그래서 재시도가 **주 1회(월요일 `--fetch-meta`)뿐**이었고, 그 경로가 고장 난 채로
  빈 값이 눌러앉았다. 이제 평일 실행도 빈 종목을 다시 조회한다
  (정상 상태에서는 대상 0개라 비용 없음)
- `item["listed_date"] = m.get("listed_date", "")`를 **`… or item.get("listed_date", "")`**로
  바꿔, 캐시가 비어 있어도 `latest.json`의 기존 값을 지우지 않는다.
  XLS 기본 경로는 원래 `if not item.get("listed_date"):`로 막혀 있었는데
  `--prices-only`만 조건 없이 덮어쓰고 있었다 (매일 도는 쪽이 하필 이쪽)

**복구:** 2026-09-07 커밋(`ec6c9cfc`)의 캐시에서 922개, 나머지 97개(종가 조회 실패 종목)는
`price_history` 첫 거래일에서 복원해 **1,019개 전부** 되살렸다.

- ⚠️ 이 사고의 교훈은 **"재취득 가능한 값과 불가능한 값을 같은 캐시에 두고 통째로 비우지 말 것"**이다.
  비슷한 초기화(`unlink`, `= {}`)를 새로 만들 때 이 구분을 먼저 확인할 것
- 네이버 HTML 스크래핑이 왜 전부 실패했는지는 아직 미확인이다(개발 환경에서
  `finance.naver.com` 접근이 403으로 막혀 재현 불가). 다음 월요일 실행 로그의
  `상장일 확보:` 줄로 네이버가 살아났는지 / 종가이력 폴백으로 버티는지 확인할 것
...
**해결(2026-08-26 재수정):** 등락률 계산을 `resolve_price_date()` **뒤로** 옮기고, 기준일보다 앞선 가장 최근 거래일 종가를 전일로 쓴다. 날짜로만 판단하며 종가 값은 비교하지 않는다.

> ⚠️ 처음 해결책(2026-08-03)은 "탐색 중 만난 종가가 현재가와 같으면 한 칸 더 앞의 거래일을 전일로 잡는다"였는데, **값이 같다는 것을 '자기 자신'의 근거로 삼은 게 잘못**이었다. 이틀 연속 같은 가격에 마감하는 종목이 실제로 있어서(2026-08-26 기준 30개) 그때마다 **하루 전 등락률이 오늘 값으로 새어 나왔다.**
>
> 2026-08-26 오류 사례: RISE 미국테크100데일리고정커버드콜(491620)이 8/25·8/26 모두 10,595원에 마감했는데, 메인 카드는 +0.57%(= 8/24 대비), 상세페이지는 0.00%로 서로 달랐다. 상세페이지(`etf.html`의 `loadLiveChg`)는 `price_history`의 마지막 두 종가를 그대로 비교하므로 이쪽이 맞았다. 총 25개 종목이 영향을 받았다.
>
> 근본 원인은 `--prices-only` 경로에서 **등락률을 `resolve_price_date()` 호출 전에 계산**하던 것이다. 그래서 보정 전 날짜(실행 당일)를 쓸 수밖에 없었고, 비거래일을 값 비교로 우회하려다 이 버그가 생겼다. 순서를 바꾸니 우회가 통째로 불필요해졌다.
>
> 검증: 921개 종목에 대해 새 로직 결과가 저장된 종가 이력과 100% 일치. 주말 실행을 시뮬레이션한 회귀 검사에서도 919개 전부 옛 로직과 동일한 값을 냈다.
...
## ⚠️ 지급일(pay_date)은 회차 안에서도 운용사별로 갈린다 — 한 종목만 보고 단정하지 말 것

**기준일(`ex_date`)은 회차 전체가 같지만, 지급일(`pay_date`)은 종목마다 다를 수 있다.**
명부확정일은 운용사끼리 공유하지만 지급 처리일은 각자 정하기 때문이다.

> 2026-08-28 오류 사례: 8월 월말 회차(기준일 2026-08-31) 콘텐츠를 쓰면서
> `eom[0]['pay_date']` 하나만 보고 "지급일은 9월 1일"이라고 단정했다.
> 실제 분포는 **9/1 16개 · 9/2 119개**로 오히려 9/2가 88%였다.
> (한국투자신탁운용 ACE 시리즈는 전부 9/2)
> 인사이트 월말 현황 글과 네이버 확정 공지 두 곳에 잘못 들어갔다.
>
> 이어서 "기준일 8/31 → 지급일 9/1이라 1영업일"을 근거로 가이드 글의
> "2~7영업일" 문구가 틀렸다고 잘못 지적하기까지 했다. 실제로는 2영업일이라 문제없었다.
> **틀린 값 하나가 다음 판단까지 연쇄로 틀리게 만든 사례.**

**콘텐츠에 지급일을 쓸 때는 반드시 분포를 확인한다:**
...
d = json.load(open('data/output/latest.json', encoding='utf-8'))
timing, ym = '월말', '2026-08'   # 해당 회차로 변경
sub = [x for x in d if x.get('current') and x.get('timing')==timing
       and x.get('freq') in ('월배당','월배당추정')
       and x.get('ex_date','').startswith(ym)]
print('기준일:', sorted(set(x['ex_date'] for x in sub)))   # 하나여야 정상
for k, v in sorted(Counter(x['pay_date'] for x in sub).items()):
    print(f'  지급일 {k}: {v}개 ({v/len(sub)*100:.0f}%)')
"
```

**실측: 최근 7개 회차가 예외 없이 두 날짜로 갈렸다** (2026-05~08, `history.json` 전수)

| 기준일 | 종목수 | 소수 | 다수 |
|---|---|---|---|
| 2026-05-29 | 125 | 06-01 (12%) | 06-02 (88%) |
| 2026-06-15 | 51 | 06-16 (12%) | 06-17 (88%) |
| 2026-06-30 | 134 | 07-01 (12%) | 07-02 (88%) |
| 2026-07-15 | 50 | 07-16 (8%) | 07-20 (92%) |
| 2026-07-31 | 399 | 08-03 (7%) | 08-04 (93%) |
| 2026-08-14 | 49 | 08-18 (8%) | 08-19 (92%) |
| 2026-08-31 | 138 | 09-01 (12%) | 09-02 (88%) |

**갈리는 것이 정상이므로, 단일 날짜로 쓰는 쪽이 오히려 예외다.**

- **갈리면(=거의 항상)** "상품에 따라 N월 N일 또는 N일"로 쓰고 다수 쪽을 밝힌다
- 지급일이 **하나로 모이는 드문 경우**에만 그 날짜를 단정한다
- 발송 전 검증 코드의 `sample = eom[0]`은 "지급일 > 기준일" 확인용이라 한 건만 봐도 되지만,
  **콘텐츠에 날짜를 인용할 때는 그 값을 쓰면 안 된다**

> 같은 원칙이 종목마다 다를 수 있는 다른 필드(`manager`, `total_fee`, `freq`)에도 적용된다.
> 회차 전체를 대표하는 값인지 아닌지부터 확인할 것.
...
### ⚠️ 월말 회차 종목 수는 결산월에 3~4배로 뛴다 — 줄었다고 오해하지 말 것

| 회차 | 종목 수 | | 회차 | 종목 수 |
|---|---|---|---|---|
| 2026-04-30 | **509** (4월 결산) | | 2026-05-29 | 125 |
| 2026-07-31 | **399** (7월 결산) | | 2026-06-30 | 134 |
| 2025-12-30 | **380** | | 2026-08-31 | 135 |
| 2026-01-30 | **286** | | 2026-02-27 | 121 |

결산이 몰리는 **4·7·12·1월**은 300~500개, 평월은 **120~135개**가 정상이다. 월중 회차는 43~71개로 비교적 일정하다.

> 2026-08-27 거짓 경보: `kind_verify.py`의 급감 검사가 **current 총계**를 비교하고 있어서, 직전 448개(월말 7/31 399 + 월중 8/14 49) 대비 현재 187개(월말 8/31 135 + 월중 8/14 49 + 비정기 3)를 42%로 계산해 **사이트 반영을 막았다.** 공시 원본 138건 = 135 + 3 으로 정확히 일치해 실제 손실은 없었다.
>
> 총계 비교는 **회차가 교체되는 시점, 즉 프로세서가 도는 바로 그때마다** 울린다. 결산월 다음 회차라면 매번이다.
>
> **수정:** 양쪽에 공통으로 있는 **기준일별로** 비교하도록 바꿨다. 같은 회차라면 종목이 줄 이유가 없으므로 원래 잡으려던 '조용한 손실'은 그대로 걸리고, 회차 교체는 오탐하지 않는다. 공통 기준일이 하나도 없으면 비교를 생략한다.

### ⚠️ 검색 페이지가 안 열리면 그날 수집 전체가 실패한다

`kind_client._open_search_page()`가 KIND 검색 화면을 열고 `#reportNmTemp`가 뜰 때까지 기다린다. 이 단계가 실패하면 그날 공시를 한 건도 못 가져온다.

> 2026-08-24 오류 사례: `kind-watch` 실행 #27이 `page.wait_for_selector("#reportNmTemp")` 30초 초과로 실패. 직전 실행(8/21)까지는 정상이었다. 다행히 8월 월말 공시 전이라 놓친 데이터는 없었으나, **공시일(기준일 2일 전쯤)에 겹쳤으면 회차를 통째로 놓쳤을 상황**이었다.

**해결 (적용됨):**
- `_open_search_page()`로 분리하고 **3회 재시도**(5초·10초 대기). 일시적 지연·차단이면 자동 복구된다
- 끝내 실패하면 예외 메시지에 **페이지 진단**을 담는다 — `url` / `title` / 본문 앞 400자 / 실제 `input` id 목록. 다음에 같은 일이 나면 로그만 보고 "느려서"인지 "화면이 바뀌어서"인지 구분할 수 있다
- `kind-watch.yml`에 **수집 실패 알림 이슈**를 추가. 종전에는 분배금 알림이 `new_files=true`일 때만 나가서 **수집 자체가 깨지면 아무 알림도 안 갔다**. 열린 이슈가 있으면 중복 생성하지 않는다
- fetch 단계는 알림을 만들려고 `continue-on-error`로 두되, 마지막 `Fail if fetch failed` 스텝에서 다시 실패로 끝낸다 (실행이 '성공'으로 잘못 표시되지 않도록)

- ⚠️ **이 저장소의 개발 환경에서는 KIND 접근이 차단(403)되어 재현 테스트가 불가능하다.** 로직 검증은 가짜 page 객체로 했고, 실제 동작은 다음 워크플로 실행으로 확인해야 한다
