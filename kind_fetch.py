...
def month_chunks(frm: str, to: str) -> list:
    ...
    ⚠️ KIND 검색에는 조용히 실패하는 제약이 둘 있어 월 단위 분할이 필수다.
       1) 결과 100건 상한 — 초과분이 잘리고 최신순이라 오래된 것부터 사라진다.
          (2018년 한 번에 검색하면 100건, 상·하반기로 나누면 100+77=177건)
       2) 기간 3년 초과 시 0건 — 오류 없이 빈 결과가 온다.
          ('이익금분배' 2005~2009 → 0건, 2009년 단독 → 77건)
       한 달치는 아무리 많아도 20건 안팎이라 두 제약 모두 걸리지 않는다.

    참고: ETF 분배금 공시는 2009-09-07이 최초이며 그 이전은 존재하지 않는다.
    """
